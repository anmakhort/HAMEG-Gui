...
void SerialConsole::handle_btnSend() {
    m_log->append("Sending <" + m_command->text() + tr("> : ") + (serial_write(_fd, m_command->text().toStdString(), "\r", 5000) == 0 ? "FALSE" : "TRUE"));
}
...
void SerialConsole::handle_btnQuery() {
    m_log->append("Query <" + m_command->text() + "> : " + QString::fromStdString(serial_query(_fd, m_command->text().toStdString(), "\r", 256)));
}
